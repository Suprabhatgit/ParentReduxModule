var ParentReduxModule = ParentReduxModule || {};

ParentReduxModule.itemViewModel = function (moduleId, resx) {
    var service = {
        path: "ParentReduxModule",
    ...
    var loadUsers = function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            var result = data[i];
            var user = new ParentReduxModule.user(result.id, result.name);
...
ParentReduxModule.user = function (id, name) {
    this.id = id;
    this.name = name;
}